...
from ..shapes.sdf_extras import sdMandelbulb, sdSierpinski, sdTree
...
def map_scene_new(p, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs):
    ...
    for i in range(num_objects):
        ...
        elif typ == SHAPE_TREE: d = sdTree(local_p)
...
def calc_normal(p, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs):
    ...
    d = map_scene_new(p, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0]
    n = vec3(
        d - map_scene_new(p - vec3(e[0], 0, 0), num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0],
        d - map_scene_new(p - vec3(0, e[0], 0), num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0],
        d - map_scene_new(p - vec3(0, 0, e[0]), num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0]
    )
    return normalize(n)
...
def calc_softshadow(ro, rd, tmin, tmax, k, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs):
    ...
    t = tmin
    for i in range(16):
        h = map_scene_new(ro + rd * t, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0]
        res = min(res, k * h / t)
        t += clamp(h, 0.02, 0.2)
...
def calc_ao(p, n, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs):
    ...
    for i in range(5):
        ...
        d = map_scene_new(p + n * h, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)[0]
        occ += (h - d) * sca
        sca *= 0.95
        if sca < 0.1: break
...
def sample_atlas_2d(uv, idx, atlas):
    ...
    width = atlas.shape[1]
    height = atlas.shape[2]

    x = int(u * width) % width
    y = int(v * height) % height
    idx_int = int(idx)

    return vec3(atlas[idx_int, y, x, 0], atlas[idx_int, y, x, 1], atlas[idx_int, y, x, 2])
...
def triplanar_sample(p, n, idx, atlas, scale):
    ...
    p = scale_vec3(p, scale)

    uv_x = np.array([p[2], p[1]], dtype=np.float32)
    uv_y = np.array([p[0], p[2]], dtype=np.float32)
    uv_z = np.array([p[0], p[1]], dtype=np.float32)

    c_x = sample_atlas_2d(uv_x, idx, atlas)
    c_y = sample_atlas_2d(uv_y, idx, atlas)
    c_z = sample_atlas_2d(uv_z, idx, atlas)

    return add_vec3(add_vec3(scale_vec3(c_x, w[0]), scale_vec3(c_y, w[1])), scale_vec3(c_z, w[2]))
...
def triplanar_normal(p, n, idx, atlas, scale):
    ...
    # Correct TBN normal mapping in triplanar is complex.
    # Let's just blend the fetched normals (which is incorrect but often sufficient for noise)
    # Assuming the texture stores normal map data.

    # For now, just return n because full triplanar normal mapping is heavy
    return n

@njit
def get_lighting(p, n, rd, mat_id, materials, num_objects, object_types, inv_matrices, scales, material_ids, operations, lights, env_map_flat, env_map_w, env_map_h, texture_atlas, aabb_mins, aabb_maxs):
    mat_idx = int(mat_id)
    ...
    total_light = np.array([0.0, 0.0, 0.0], dtype=np.float32)

    # Loop over lights
    # lights shape: (N, 7) -> [x, y, z, r, g, b, intensity]
    num_lights = lights.shape[0]

    for i in range(num_lights):
        light_pos = vec3(lights[i, 0], lights[i, 1], lights[i, 2])
        light_col = vec3(lights[i, 3], lights[i, 4], lights[i, 5])
        light_int = lights[i, 6]

        # Directional Light approximation (since positions are far)
        # Or point light?
        # The original code treated it as directional sun_dir = normalize(vec3(0.5, 0.8, 0.5))
        # But lights array has Pos. Let's assume point light if close, directional if very far?
        # For compatibility with "Studio" setup in tools, let's treat them as Directional Lights pointing to origin?
        # No, "Pos" implies position.

        # Let's use Directional for now as that's what the shadows support easily
        # Direction from light to origin? No, usually light direction vector.
        # But [5, 8, 5] looks like a position.

        l_dir = normalize(light_pos) # Treat as direction vector towards light

        # Shadows (Only for the first light / Key light for performance?)
        # Or accumulate shadows?
        # Soft shadow is expensive. Let's do it only for the first light (Key)
        shadow = 1.0
        if i == 0:
             shadow = calc_softshadow(p, l_dir, 0.02, 3.0, 8.0, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, aabb_mins, aabb_maxs)

        h = normalize(add_vec3(l_dir, scale_vec3(rd, -1.0)))
        h_dot_l = clamp(dot(h, l_dir), 0.0, 1.0)
        ...
        fresnel_term = float(pow(1.0 - h_dot_l, 5.0))
        ...
        light_contrib = scale_vec3(mul_vec3(add_vec3(diffuse, specular), light_col), light_int * shadow)
        total_light = add_vec3(total_light, light_contrib)

        # CLEAR COAT
        if clear_coat > 0.0:
            cc_roughness = 0.05
            cc_f0 = 0.04
            cc_fresnel_term = float(pow(1.0 - h_dot_l, 5.0))
            ...
            cc_specular = scale_vec3(cc_specular, ndotl * shadow * light_int * clear_coat)
            attenuation = 1.0 - (cc_fresnel * clear_coat * 0.5)
            # This attenuation logic is a bit simplified for multi-light but acceptable
            # We should probably accumulate clear coat separately
            total_light = add_vec3(total_light, cc_specular)

    col = add_vec3(col, total_light)
    ...
    if sss_strength > 0.0:
        distortion = 0.5
        # Approximation: Use first light as "sun" for SSS direction
        sun_dir_approx = normalize(vec3(0.5, 0.8, 0.5)) # Fallback
        if lights.shape[0] > 0:
            sun_dir_approx = normalize(vec3(lights[0, 0], lights[0, 1], lights[0, 2]))

        trans_light = add_vec3(sun_dir_approx, scale_vec3(n, distortion))
...
@njit
def raymarch_kernel(ro, rd, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, lights, env_map_flat, env_map_w, env_map_h, texture_atlas, aabb_mins, aabb_maxs):
    dO = 0.0
    ...
    for i in range(MAX_STEPS):
        ...
        if d < SURF_DIST:
            ...
            col = get_lighting(p, n, rd, m, materials, num_objects, object_types, inv_matrices, scales, material_ids, operations, lights, env_map_flat, env_map_w, env_map_h, texture_atlas, aabb_mins, aabb_maxs)
...
def render_pixels(width, height, cam_pos, cam_target, fov,
    ...
    for y in prange(height):
        for x in range(width):
            ...
            col = raymarch_kernel(ro, rd, num_objects, object_types, inv_matrices, scales, material_ids, operations, materials, lights, env_map_flat, env_map_w, env_map_h, texture_atlas, aabb_mins, aabb_maxs)
            r_acc += col[0]
            g_acc += col[1]
            b_acc += col[2]
...
# Engine Class Wrapper to manage state
class Engine:
    def __init__(self, width, height):
        ...
        self.lights = []
        self.materials = []
        self.env_map = np.zeros((1, 1, 3), dtype=np.float32) # Default black

        # Add default materials if needed

    def add_object(self, shape_type, params, transform, material, operation=OP_UNION):
        self.objects.append({
            'type': shape_type,
            'params': params,
            'transform': transform,
            'material': material, # Material PROPERTIES array (not ID)
            'op': operation
        })
    ...
    def set_env_map(self, env_map_data):
        self.env_map = env_map_data
    ...
    def render(self):
        ...
        inv_matrices = np.zeros((num_objects, 4, 4), dtype=np.float32)
        scales = np.zeros((num_objects, 3), dtype=np.float32)
        material_ids = np.zeros(num_objects, dtype=np.float32)
        operations = np.zeros(num_objects, dtype=np.float32)
        aabb_mins = np.zeros((num_objects, 3), dtype=np.float32)
        aabb_maxs = np.zeros((num_objects, 3), dtype=np.float32)

        # Collect materials
        # For simplicity, let's say each object has its own material, so materials array matches objects
        # Or we can deduplicate.
        # But the engine expects material_ids pointing to a materials array.

        # Let's stack materials
        materials_list = [obj['material'] for obj in self.objects]
        # Pad materials to 22 floats
        padded_materials = []
        for mat in materials_list:
            padded = np.zeros(22, dtype=np.float32)
            length = min(len(mat), 22)
            padded[:length] = mat[:length]
            padded_materials.append(padded)

        materials_array = np.array(padded_materials, dtype=np.float32)

        for i, obj in enumerate(self.objects):
            object_types[i] = obj['type']

            # Inverse Matrix
            mat = obj['transform']
            inv_mat = np.linalg.inv(mat).astype(np.float32)
            inv_matrices[i] = inv_mat

            # Scale (extracted from params or matrix? The engine seems to expect scale in 'scales' array)
            # The low-level engine uses 'scales' differently per shape.
            # Sphere: scales[0] = radius
            # Box: scales = dimensions
            # Tree: no params needed, but we pass empty?

            # We need to map 'params' to 'scales'
            p = obj['params']
            ...
            material_ids[i] = float(i) # Direct mapping
            operations[i] = float(obj['op'])

            # AABB (Placeholder - Infinite)
            aabb_mins[i] = np.array([-1000, -1000, -1000], dtype=np.float32)
            aabb_maxs[i] = np.array([1000, 1000, 1000], dtype=np.float32)

        # Lights
        lights_array = np.zeros((len(self.lights), 7), dtype=np.float32)
        for i, l in enumerate(self.lights):
            lights_array[i, 0:3] = l['pos']
            lights_array[i, 3:6] = l['color']
            lights_array[i, 6] = l['intensity']

        # Env Map
        env_map_flat = self.env_map.flatten()
        env_map_w = self.env_map.shape[1]
        env_map_h = self.env_map.shape[0]

        # Texture Atlas (Placeholder)
        texture_atlas = np.zeros((1, 1, 1, 3), dtype=np.float32)

        # Output Buffer
        output_buffer = np.zeros(self.width * self.height * 3, dtype=np.float32)

        render_scene(self.width, self.height, self.camera_pos, self.camera_target, self.fov,
                    num_objects, object_types, inv_matrices, scales, material_ids, operations,
                    materials_array, lights_array, env_map_flat, env_map_w, env_map_h, texture_atlas, aabb_mins, aabb_maxs, output_buffer)

        return output_buffer.reshape((self.height, self.width, 3))
