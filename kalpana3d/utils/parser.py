...
def calculate_aabb(typ, scale, world_mat):
    ...
    elif typ == 11: # RoundCone
        # r1=scale[0], r2=scale[1], h=scale[2]
        r1, r2, h = scale[0], scale[1], scale[2]
        r_max = max(r1, r2)
        # Bounding box of a cone along Y from 0 to h
        # x,z in [-r_max, r_max], y in [0, h]
        # plus padding for r1, r2
        lx, ly, lz = r_max, h, r_max
        # Center of that box is 0, h/2, 0. Extents: r_max, h/2, r_max
        # But our AABB func assumes centered at origin for radius calc?
        # Let's just use conservative sphere
        r_bound = max(h, r_max*2)
        lx, ly, lz = r_bound, r_bound, r_bound
    else:
