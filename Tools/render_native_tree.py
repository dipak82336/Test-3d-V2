...
def render_view(scene, angle_name, cam_pos_override=None, output_path="output.png"):
    width = 320 # Low quality for speed
    height = 240
...
def main():
    scene_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../kalpana3d/example/tree_native_sdf.yaml'))
    if not os.path.exists(scene_path):
        # Fallback to local
        scene_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../example/tree_native_sdf.yaml'))
    ...
    views = {
        "native_tree_front": [0.0, height, dist],       # Front (Z+)
        "native_tree_back": [0.0, height, -dist],      # Back (Z-)
        "native_tree_left": [-dist, height, 0.0],      # Left (X-)
        "native_tree_right": [dist, height, 0.0]       # Right (X+)
    }
